#!/usr/bin/env python3
"""
Create synthetic SEVIR-like test data and test our model
"""
...
def create_synthetic_sevir_data(num_samples=10):
    """Create more synthetic SEVIR-like radar data"""
    print(f"🌩️  Creating synthetic SEVIR-like test data ({num_samples} samples)...")
    ...
    for sample in range(num_samples):
        ...
        for t in range(25):
            ...
            for storm in storms:
                ...
                # Create storm with synthetic radar characteristics
            ...
            # Add synthetic noise and artifacts
            frame += np.random.gamma(1, 2, (384, 384)) * 0.5  # Ground clutter
            frame = np.clip(frame, 0, 75)  # Synthetic dBZ range
            
            # Apply beam blockage and range effects (synthetic radar limitations)
    ...
    print(f"✅ Created synthetic SEVIR-like data:")
...
def save_as_sevir_format(input_data, output_data, filename="synthetic_sevir_test.h5"):
...
def test_model_on_synthetic_data():
    """Test our model on synthetic SEVIR-like data"""
    print("🌩️  Testing Model on Synthetic SEVIR-like Data")
    print("=" * 60)
    
    # Create synthetic test data
    input_data, output_data = create_synthetic_sevir_data(num_samples=5)
    
    # Save in SEVIR format
    test_file = save_as_sevir_format(input_data, output_data, "data/sample/synthetic_sevir_test.h5")
    ...
    # Visualize results
    visualize_synthetic_results(input_data, output_data, y_pred_dbz, sample_idx=0)
    ...
    print(f"\n🎯 ASSESSMENT:")
    if mae < 0.15:  # < ~7 dBZ
        print("🌟 Excellent performance on synthetic weather data!")
    elif mae < 0.25:  # < ~12 dBZ
        print("👍 Good performance on synthetic weather data!")
...
def visualize_synthetic_results(input_data, output_data, predictions, sample_idx=0):
    """Visualize results on synthetic data"""
    ...
    filename = f"synthetic_sevir_test_{timestamp}.png"
